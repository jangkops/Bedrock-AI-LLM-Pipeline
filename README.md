...
AWS Bedrock 모델 호출을 중앙 관리하는 서버리스 게이트웨이. 사용자별 KRW 비용 기반 월간 한도, 승인 워크플로, 실시간 사용량 추적, 비동기 장시간 호출(Fargate) 지원.
...
.
├── README.md
├── infra/bedrock-gateway/          # Terraform IaC (전체 인프라)
│   ├── main.tf                     # API Gateway REST API (Regional, AWS_IAM)
│   ├── lambda.tf                   # Lambda 함수 + alias
│   ├── iam.tf                      # Lambda/Fargate/SFN IAM roles + policies
...
│   └── worker/
│       ├── main.py                 # 실제 Bedrock 호출 Fargate worker
│       ├── main_synthetic.py       # 테스트용 synthetic worker
│       └── Dockerfile              # Python 3.12-slim + boto3
├── client/                         # 클라이언트 도구
│   ├── bedrock_gw.py               # Python 클라이언트 (boto3 호환 wrapper)
│   ├── bedrock-gw-quota-check.sh   # Shell hook (승인 프롬프트)
│   ├── bedrock-gw-request.sh       # CLI 요청 도구
│   └── bedrock-gw-status.sh        # CLI 상태 조회 도구
├── docs/ai/                        # 설계/검증/운영 문서
│   ├── research.md                 # 아키텍처 리서치
│   ├── plan.md                     # 구현 계획
│   ├── gateway-v2-spec.md          # v2 async 스펙
│   ├── v2-async-fargate-final-report.md
│   └── ...                         # phase별 검증/운영 문서
└── .kiro/
    ├── specs/bedrock-access-gateway/  # 요구사항/설계/태스크 스펙
    └── steering/                      # 운영 모델/기술 스택 가이드
```
...
| 리소스 | ID/ARN |
|--------|--------|
| API Gateway REST (Regional) | `5l764dh7y9` |
...
| ECR Repository | `107650139384.dkr.ecr.us-west-2.amazonaws.com/bedrock-gw-dev-worker` |
| Step Functions | `bedrock-gw-dev-job-orchestrator` (Standard) |
| S3 Bucket | `bedrock-gw-dev-payload-107650139384` |
| VPC | `vpc-0b52ff4687fde0661` |
| Private Subnets | `subnet-0809ebd78013e946b`, `subnet-06d9f3a6efddd8a67` |
| Security Group | `sg-0d9246fcbbc10ae9b` |
| CloudWatch Log Groups | `/aws/lambda/bedrock-gw-dev-gateway`, `/aws/apigateway/bedrock-gw-dev-api/access`, `/aws/ecs/bedrock-gw-dev-worker`, `/aws/states/bedrock-gw-dev-job-orchestrator` |
...
| `model-pricing` | `model_id` | — | 모델별 KRW 가격 (관리자 관리, 115개+) |
...
- Container Image: `107650139384.dkr.ecr.us-west-2.amazonaws.com/bedrock-gw-dev-worker:latest`
- Log Group: `/aws/ecs/bedrock-gw-dev-worker` (retention 90일)

### Worker 컨테이너 환경변수 (Task Definition에 설정)
...
- ECS: `RunTask`, `StopTask`, `DescribeTasks` (gateway cluster only, condition: ArnEquals)
- IAM: `PassRole` (fargate-exec, fargate-task)
- DynamoDB: job-state, concurrency-semaphore (read/write)
- CloudWatch Logs: log delivery 관리
- EventBridge: rule 관리

### 5. API Gateway CloudWatch Role (optional)
- `AmazonAPIGatewayPushToCloudWatchLogs`
- `manage_api_gateway_account_cloudwatch_role` 변수로 제어
...
| 유형 | 동작 |
|------|------|
| Gateway-managed | `principal_policy`에 등록, 한도/승인 적용 |
| Exception (direct-use) | 직접 Bedrock 접근, CloudWatch 모니터링만, `EXCEPTION_USERS` dict |

분류는 운영자의 명시적 결정. 자동 전환 없음.

### 데이터 키 패턴

- Model pricing: strict `model_id` match, no normalization
...
| `lambda_runtime` | `python3.12` | Lambda 런타임 |
| `lambda_timeout` | `900` | Lambda 타임아웃 (초) |
| `lambda_memory_size` | `256` | Lambda 메모리 (MB) |
| `api_gateway_stage_name` | `v1` | API Gateway 스테이지명 |
| `api_throttle_rate_limit` | `50` | API 초당 요청 제한 |
| `api_throttle_burst_limit` | `100` | API 버스트 제한 |
| `ses_sender_email` | (필수) | SES 발신 이메일 |
| `ses_admin_group_email` | (필수) | 관리자 이메일 |
| `discovery_mode` | `false` | principal 캡처 모드 |
| `manage_api_gateway_account_cloudwatch_role` | `false` | API GW CW role 관리 여부 |
| `api_gateway_execution_logging_level` | `ERROR` | API GW 로깅 레벨 (OFF/ERROR/INFO) |
| `api_gateway_data_trace_enabled` | `false` | API GW 데이터 트레이스 |
| `private_subnet_ids` | `[]` | Fargate private subnet IDs |
| `fargate_security_group_ids` | `[]` | Fargate security group IDs |
...
aws ecr get-login-password --region us-west-2 | \
  docker login --username AWS --password-stdin 107650139384.dkr.ecr.us-west-2.amazonaws.com
docker build -t bedrock-gw-dev-worker .
docker tag bedrock-gw-dev-worker:latest \
  107650139384.dkr.ecr.us-west-2.amazonaws.com/bedrock-gw-dev-worker:latest
docker push 107650139384.dkr.ecr.us-west-2.amazonaws.com/bedrock-gw-dev-worker:latest
```
...
```python
from client.bedrock_gw import BedrockGatewayClient

client = BedrockGatewayClient(region='us-west-2', environment='dev')
...
```bash
# /etc/profile.d/bedrock-gw.sh 에 설치
source client/bedrock-gw-quota-check.sh
```
...
- No-bypass (Bedrock/ECS/SFN direct access blocked): PASS

## 관련 문서

- `docs/ai/research.md` — 아키텍처 리서치 및 의사결정
- `docs/ai/plan.md` — 구현 계획
- `docs/ai/gateway-v2-spec.md` — v2 async Fargate 스펙
- `docs/ai/v2-async-fargate-final-report.md` — v2 최종 검증 보고서
- `docs/ai/long-running-bedrock-architecture-final.md` — long-running 아키텍처 분석
- `.kiro/specs/bedrock-access-gateway/` — 요구사항, 설계, 태스크 스펙
