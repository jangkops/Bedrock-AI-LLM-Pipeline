...
Long-running models (Opus variants) hitting `/converse` are silently routed to `handle_converse_job_submit` → returns 202.
...
- `DenyDirectBedrockInference`: Deny bedrock:InvokeModel, InvokeModelWithResponseStream, Converse, ConverseStream on *
...
- `AllowDevGatewayConverseJobs`: Allow execute-api:Invoke on GET/POST converse-jobs/*
...
Exception users: `DenyDirectBedrockInference` removed, `BedrockAccess` upgraded with `bedrock:InvokeTool`.
