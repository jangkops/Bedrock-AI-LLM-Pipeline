...
| Item | Value |
|---|---|
| AWS Account | `107650139384` |
| Region | `us-west-2` |
| User A role | `arn:aws:iam::107650139384:role/BedrockUser-jwlee` |
| User B role | `arn:aws:iam::107650139384:role/BedrockUser-shlee2` |
| Fail-closed role | `arn:aws:iam::107650139384:role/BedrockUser-Shared` |
...
```json
{
  ...
  "Statement": [{
    ...
    "Resource": "arn:aws:execute-api:us-west-2:107650139384:<api-id>/discovery/POST/*"
  }]
}
```
...
aws sts assume-role \
  --role-arn arn:aws:iam::107650139384:role/BedrockUser-jwlee \
...
```json
{
  ...
  "Arn": "arn:aws:sts::107650139384:assumed-role/BedrockUser-jwlee/laptop-discovery"
}
```
...
```json
{
  "discovery": true,
  "raw_identity": {
    "userArn": "arn:aws:sts::107650139384:assumed-role/BedrockUser-jwlee/laptop-discovery",
  ...
  "derived_principal_id": "arn:aws:sts::107650139384:assumed-role/BedrockUser-jwlee/laptop-discovery"
}
```
...
- `account_id`: the fifth `:`-delimited segment (e.g. `107650139384`)

Compute Candidate F: `{account_id}#{role_name}` → e.g. `107650139384#BedrockUser-jwlee`
...
# Expected:
# [default]
# role_arn = arn:aws:iam::107650139384:role/BedrockUser-jwlee
...
```json
{
  ...
  "Arn": "arn:aws:sts::107650139384:assumed-role/BedrockUser-jwlee/<session-name>"
}
```
...
| `account_id` | `107650139384` | `107650139384` | YES |
| Candidate F | `107650139384#BedrockUser-jwlee` | `107650139384#BedrockUser-jwlee` | YES — must be identical |
...
| C3 Candidate F | `107650139384#BedrockUser-shlee2` (different from C1/C2) |
...
aws sts assume-role \
  --role-arn arn:aws:iam::107650139384:role/BedrockUser-Shared \
...
~~현재 discovery Lambda (`ugpt5xi8b7`)는 placeholder `normalize_principal_id()`를 사용 중:~~
~~- `derived_principal_id` = full userArn (session name 포함)~~
~~- C1 evidence에서 확인: `"derived_principal_id": "arn:aws:sts::107650139384:assumed-role/BedrockUser-cgjang/botocore-session-1773732261"`~~

**재배포 완료 (2026-03-17):**
- `derived_principal_id` = `107650139384#BedrockUser-cgjang` (Candidate F) — live 확인
...
awscurl --service execute-api --region us-west-2 \
  -X POST \
  -d '{"modelId":"discovery-test"}' \
  "https://ugpt5xi8b7.execute-api.us-west-2.amazonaws.com/v1/discovery"
```
...
|---|---|---|
| `derived_principal_id` | `107650139384#BedrockUser-cgjang` | ✅ |
...
|---|---|
| `handler.py:84-96` `normalize_principal_id()` | Candidate F IMPLEMENTED — returns `<account>#<role-name>`. Fail-closed on non-BedrockUser/Shared roles. Live-verified: `107650139384#BedrockUser-cgjang`. |
